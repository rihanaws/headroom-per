...
| 14 | (implements deferred part of #13) Manual-review approval is a CLI script run by hand against `manual-review.json`, not a server endpoint — operator lists pending entries, picks one, chooses deactivate-previous / keep-both / dismiss. The superseded observation is never deleted or replaced with a new row; the existing `is_active` boolean on the `observations` table (already present in `knowledge-base/src/db.ts`, already filtered by every read path — `memory-adapter.ts`, `refinement.ts`, `docs-generator.ts`, `/graph`) is flipped to 0 via the same `UPDATE observations SET is_active = 0 WHERE id = ?` pattern `refinement.ts` already uses. No schema change. Undo is the same script with a `--restore` flag flipping the row back to `is_active = 1` — never a raw-SQL-by-hand procedure. | A new-row-plus-`superseded_by` design was considered and rejected: it adds two columns and a version-chain concept to reason about on every read path, to solve a conflict volume of one (synthetic). `is_active` already exists, is already indexed, is already the filter every retrieval path uses — reusing it is strictly cheaper than introducing a parallel versioning scheme, and a real version-chain can still be layered on top later if conflict volume ever justifies it. Undo via raw SQLite access (no script) was rejected because it has no audit trail and depends on remembering exact syntax under time pressure — both at odds with this project's "actually verify, don't infer" discipline. |
